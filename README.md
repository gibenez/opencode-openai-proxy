...
- `function_call_output` validation is strict (unknown `call_id` rejected, duplicate submissions rejected); continuation can be resolved by `previous_response_id` or inferred by `call_id` for LangChain flows.
